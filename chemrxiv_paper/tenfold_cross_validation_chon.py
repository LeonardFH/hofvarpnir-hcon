...
# CHON FILTER TOGGLE
# ==============================================================================
# Set to True to restrict datasets to ONLY Carbon, Hydrogen, Oxygen, Nitrogen.
# Set to False to use the full, unfiltered datasets.
FILTER_CHON = True  
ALLOWED_ATOMS = {"C", "H", "O", "N"}

def is_chon(smiles):
    """Returns True if the molecule contains ONLY C, H, O, N."""
    mol = Chem.MolFromSmiles(smiles)
    if not mol:
        return False
    return {atom.GetSymbol() for atom in mol.GetAtoms()}.issubset(ALLOWED_ATOMS)
...
def run_dataset_cv(dataset_name, csv_file):
    ...
    # --------------------------------------------------------------------------
    # APPLY CHON FILTER (IF TOGGLED)
    # --------------------------------------------------------------------------
    if FILTER_CHON:
        print("🧪 Applying CHON filter...")
        chon_mask = df["SMILES"].apply(is_chon)
        original_len = len(df)
        df = df[chon_mask].reset_index(drop=True)
        print(f"✅ Retained {len(df):,} CHON molecules (removed {original_len - len(df):,} non-CHON).")
    ...
    # Smart filename based on toggle
    suffix = "_chon" if FILTER_CHON else ""
    prediction_filename = (
        f"{dataset_name.lower()}_10fold_unseen_results{suffix}.csv"
    )
...
def main():
    ...
    if FILTER_CHON:
        print("🧪 MODE: CHON FILTER ACTIVE (Only C, H, O, N molecules retained)")
    else:
        print("🧪 MODE: FULL DATASETS (No elemental filtering)")
    print("=" * 100)
    print()
    ...
    paper_df.to_csv(
        f"tenfold_dataset_comparison{suffix}.csv",
        index=False
    )
    ...
    fold_df.to_csv(
        f"tenfold_all_fold_results{suffix}.csv",
        index=False
    )
    ...
    print(
        f"💾 Saved comparison table:"
        f" tenfold_dataset_comparison{suffix}.csv"
    )

    print(
        f"💾 Saved complete fold results:"
        f" tenfold_all_fold_results{suffix}.csv"
    )
